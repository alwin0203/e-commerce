...
def index(request):
    homeitems1=Item.objects.filter(collection_id='2')
    homeitems2=Item.objects.filter(collection_id ='1')  
    homeitems3=Item.objects.filter(collection_id ='3')
...
def product(request,id):
    homeitem=Item.objects.get(id=id)
    homeitemsall=Item.objects.all()
    return render (request,'shop-single.html',{'homeitem':homeitem,'homeitemsall':homeitemsall } )
...
def view_cart(request):
    ...
    grand_total = 0
    for i in cart_items:
        print(f"==================={i.product.item_price_offer} {i.quantity}")
        grand_total = grand_total + i.product.item_price_offer * i.quantity
    print(grand_total)
...
def check_out(request):
    ...
    return render(request, 'shop-checkout.html', {'cart_items': cart_items, 'grand_total': grand_total,'qty':qty })
